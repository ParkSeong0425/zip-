...
#define HOME_TRIG       0U
#define HOME_DIR        1U      /* reversed as requested: 0=CW, 1=CCW */
...
/* Original transfer timing: no 1 ms delay after RX mode */
static int xfer(
        uint8_t *tx,
        uint16_t tn,
        uint8_t *rx,
...
/*
 * F1 raw diagnostic.
 * Receives one byte at a time so timeout/partial/wrong frames can be seen.
 */
static int probe_f1(uint8_t *state)
{
    ...
    /* Wait longer only for the first byte. */
    st = HAL_UART_Receive(&huart5, &rx[0], 1, 500);
    if (st == HAL_OK) {
        n = 1;

        /* Remaining bytes should arrive back-to-back. */
...
int mks_init(void)
{
    ...
    /*
     * The compact project can reach this point before the MKS boot is over.
     * Wait, retry, and print the actual RX bytes instead of only "F1 FAIL".
     */
    HAL_Delay(1500);

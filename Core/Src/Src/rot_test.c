...
int mks_stop(void) { return write_axis(C_ABS, 0, MOVE_ACC, 0); }
...
static int probe(uint8_t *state)
{
    ...
    if (HAL_UART_Transmit(&huart5, tx, 4, 100) != HAL_OK) return 0;

    while (__HAL_UART_GET_FLAG(&huart5, UART_FLAG_TC) == RESET) { }
    for (volatile uint32_t i = 0; i < 100; i++) { }
    HAL_GPIO_WritePin(rs4852_GPIO_Port, rs4852_Pin, GPIO_PIN_RESET);

    if (HAL_UART_Receive(&huart5, &rx[0], 1, 500) != HAL_OK) return 0;

    for (n = 1; n < (int)sizeof(rx); n++)
        if (HAL_UART_Receive(&huart5, &rx[n], 1, 10) != HAL_OK) break;
...
int mks_init(void)
{
    ...
    if (mode != MODE_SR_VFOC) {
        if (!set1(C_MODE, MODE_SR_VFOC)) { print("MKS 82 FAIL\r\n"); return 0; }
        HAL_Delay(100);
    }

    /* 128분주에서는 명령 속도가 1/8로 줄어드므로 16분주로 맞춘다 */
    if (mstep != 16U && mstep != 32U && mstep != 64U) {
        if (!set1(C_MSTEP, NORMAL_MSTEP)) { print("MKS 84 FAIL\r\n"); return 0; }
        HAL_Delay(100);
    }

...
static char name(Motor *m) { return (m == &motorX) ? 'X' : 'Y'; }

static int *mode_ptr(Motor *m) { return (m == &motorX) ? &x_mode : &y_mode; }
...
static int write16(Motor *m, uint16_t reg, uint16_t val)
{
    ...
    if (bus_xfer(m, tx, 8, rx, 8) != HAL_OK ||
        memcmp(rx, tx, 6) || rx[6] != tx[6] || rx[7] != tx[7]) {
...
static int read16(Motor *m, uint16_t reg, uint16_t *out)
{
    ...
    if (bus_xfer(m, tx, 8, rx, 7) != HAL_OK) return 0;

    crc = crc16(rx, 5);

    if (rx[0] != ID || rx[1] != 0x03 || rx[2] != 2 ||
        rx[5] != (uint8_t)crc || rx[6] != (uint8_t)(crc >> 8)) return 0;
...
static int write32(Motor *m, uint16_t reg, int val)
{
    ...
    return rx[0] == ID && rx[1] == 0x10 &&
           rx[2] == tx[2] && rx[3] == tx[3] &&
           rx[4] == 0 && rx[5] == 2 &&
           rx[6] == (uint8_t)crc && rx[7] == (uint8_t)(crc >> 8);
}
...
static int read32(Motor *m, uint16_t reg, int *out)
{
    ...
    if (rx[0] != ID || rx[1] != 0x03 || rx[2] != 4 ||
        rx[7] != (uint8_t)crc || rx[8] != (uint8_t)(crc >> 8)) return 0;
...
static int check(Motor *m)
{
    ...
    for (int i = 0; i < 2; i++) {
        if (read16(m, R_ADDR, &v) && v == ID) return 1;
        HAL_Delay(30);
    }
    return 0;
}
...
int motor_move(Motor *m, int rpm, int target)
{
    ...
    if (!write32(m, R_POS, target - m->off))     return 0;

...
typedef enum {
	S_IDLE, S_C1_START, S_C1_WAIT, /* 1. ROT -> C */
...
/* 저장한 좌표와 이 기계의 보정값을 그대로 목표로 사용한다. */
static int tx(int i) {
	return data.x[i] + data.x_adj;
}
static int ty(int i) {
	return data.y[i] + data.y_adj;
}
static int in_tx(void) {
	return data.in_x + data.x_adj;
}
static int in_ty(void) {
	return data.in_y + data.y_adj;
}

static int grid_ok(void) {
	return data.x_n >= 2 && data.x_n <= MAX_X && data.y_n >= 2
			&& data.y_n <= MAX_Y;
}
...
static void data_read(void) {
	...
	if (data.x_n < 0 || data.x_n > MAX_X || data.y_n < 0 || data.y_n > MAX_Y)
		data_clear();
}

/* ===== ROT ===== */

/* ROT 정지 또는 목표 위치가 연속 3회 확인되면 완료 */
static int wait_rot(int target) {
	...
	while (HAL_GetTick() - t0 < TIMEOUT) {
		...
		if (pok && gap(now, target) <= ROT_GAP)
			stable++;
		else
			stable = 0;

		/* 정상 정지 확인 또는 목표 위치 3회 연속 확인 */
		if (pok && gap(now, target) <= ROT_GAP) {
			if ((sok && state == 1U) || stable >= 3)
				return 1;
		}
	...
	snprintf(msg, sizeof(msg),
			"ROT WAIT FAIL target=%d state=%u pos=%d gap=%d\r\n", target, state,
			now, gap(now, target));
	reply(msg);
...
static int move_rot(int t) {
	return mks_move_abs(ROT_RPM, t) && wait_rot(t);
}
...
static int home_one(Motor *m, int rpm) {
	...
	if (!motor_speed_mode(m) || !motor_speed(m, rpm))
		return 0;
...
static int save_start(int axis, int n) {
	...
	if (save_busy() || save_axis || !grid_ok())
		return 0;
	...
	if (axis == AXIS_X) {
		...
		if (n != data.x_n && n != data.x_n - 1)
			return 0;
		/* 마지막 전 열은 마지막 열이 저장된 뒤에만 */
		if (n == data.x_n - 1 && !data.x_ref)
			return 0;
		m = &motorX;
		...
		if (!data.x_ready || !home_one(&motorX, -HOME_RPM))
			return 0;
		m = &motorX;
		rpm = HOME_RPM;
	} else if (axis == AXIS_ADJ_Y) {
		if (!data.y_ready || !home_one(&motorY, HOME_RPM))
			return 0;
		m = &motorY;
		rpm = -HOME_RPM;
	} else
...
static int save_end(void) {
	...
	if (!motor_speed(m, 0))
		goto bad;
	HAL_Delay(100);
	if (!motor_pos(m, &pos) && !motor_pos(m, &pos))
		goto bad;
	...
	if (axis == AXIS_ADJ_X) {
		...
		/* 두 번째 저장: 간격을 구해 마지막 단까지 계산 */
			...
			for (int i = 1; i < data.y_n; i++)
				data.y[i] = data.y[0] + step * i;
			data.y_ready = 1;
		} else
			goto bad;
	}
...
static int seq_start(int col, int dan) {
	if (!grid_ok() || !data.x_ready || !data.y_ready)
		return 0;
	if (data.in_ready != IN_OK)
		return 0;
	if (save_busy() || save_axis || !rot_homed || !x_homed || !y_homed)
		return 0;
	if (col < 1 || col > data.x_n || dan < 1 || dan > data.y_n)
		return 0;
...
static void seq_run(void) {
	...
	case S_IN_START:
		if (!held())
			return;
		if (!xy_start(in_tx(), in_ty())) {
			seq_fail("IN START FAIL\r\n");
			return;
		}
		seq_next(S_IN_WAIT);
		return;
...
static int parse_save_cmd(const char *cmd, char axis, int *n) {
	...
	if (cmd[0] != axis)
		return 0;
	if (sscanf(&cmd[1], "%d%15s", n, rest) < 1)
		return 0;
...
static void show(void) {
	...
	for (int i = 0; i < data.x_n && k < (int) sizeof(msg) - 24; i++)
		k += snprintf(&msg[k], sizeof(msg) - k, " %d=%dmm", i + 1,
				MM(data.x[i]));
	...
	snprintf(&msg[k], sizeof(msg) - k,
			...
			MM(data.in_y), MM(data.x_adj), MM(data.y_adj), data.x_ready,
			data.y_ready, data.in_ready);
	reply(msg);
}
...
static int load(const char *s) {
	...
	if (sscanf(s, "%d %d%n", &d.x_n, &d.y_n, &n) != 2)
		return 0;
	if (d.x_n < 2 || d.x_n > MAX_X || d.y_n < 2 || d.y_n > MAX_Y)
		return 0;
	s += n;
	...
	if (sscanf(s, "%d %d", &d.in_x, &d.in_y) != 2)
		return 0;
...
static void status(void) {
	...
	snprintf(msg, sizeof(msg), "grid=%d,%d rot_home=%d x_home=%d y_home=%d "
			"x_ready=%d y_ready=%d in_ready=%d "
			"rot=%d x=%d y=%d step=%d,%d busy=%d save=%d\r\n", data.x_n,
			data.y_n, rot_homed, x_homed, y_homed, data.x_ready, data.y_ready,
			data.in_ready, rot, x, y, run_col, run_dan, save_busy(), save_axis);
	reply(msg);
}
...
static void command(char *cmd) {
	...
	if (!strcmp(cmd, "rot c")) {
		reply(rot_cmd(C_POS) ? "OK C\r\n" : "ERR C\r\n");
		return;
	}
	if (!strcmp(cmd, "rot r")) {
		reply(rot_cmd(R_POS) ? "OK R\r\n" : "ERR R\r\n");
		return;
	}
	if (!strcmp(cmd, "rot l")) {
		reply(rot_cmd(L_POS) ? "OK L\r\n" : "ERR L\r\n");
		return;
	}
	...
	if ((cmd[0] == 'x' || cmd[0] == 'y' || !strncmp(cmd, "go", 2)
			|| !strncmp(cmd, "in", 2) || !strncmp(cmd, "adj", 3))
			&& !grid_ok()) {

...
static int ok_code(char c) { return c >= FIRST && c <= LAST; }

/* ===== 예약 큐 ===== */

static int q_empty(void) { return q_head == q_tail; }

static void q_clear(void) { q_head = q_tail = 0; }

static int q_push(char c)
{
    int next = (q_tail + 1) % QUEUE_N;

    if (next == q_head) return 0;   /* 꽉 참 */
...
static void fill_all(void)
{
    ...
    if (!seeded) { srand(HAL_GetTick()); seeded = 1; }
    ...
    for (int d = 0; d < yn; d++) {
        for (int c = 0; c < xn; c++) {
            ...
            for (int i = 0; i < n; i++) cnt[d][c][rand() % CODE_N]++;
        }
    }
}
...
static void cell_text(int d, int c, char *out)
{
    ...
    for (int i = 0; i < CODE_N; i++)
        for (int j = 0; j < cnt[d][c][i]; j++) out[k++] = FIRST + i;
...
int item_cmd(char *cmd)
{
    ...
    if (!strcmp(cmd, "start")) {
        if (save_busy()) { reply("ERR RUNNING\r\n"); return 1; }
    ...
    if (!strcmp(cmd, "map")) { show_map(); return 1; }

    /* a ~ g 한 글자. 진행중이면 예약해 둔다 */
    if (ok_code(cmd[0]) && !cmd[1]) {
        if (!q_push(cmd[0])) reply("QUEUE FULL\r\n");
        else {
